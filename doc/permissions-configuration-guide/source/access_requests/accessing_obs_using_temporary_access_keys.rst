...
When an IAM user calls the IAM API to `obtain a temporary AK/SK <https://docs.otc.t-systems.com/en-us/api/iam/en-us_topic_0097949518.html>`__, they can include the **policy** parameter to attach a temporary policy that further restricts the permissions that may be granted to other users. Temporary policies must follow the same format and content requirements as those defined in :ref:`IAM Permissions <obs_40_0003>`.

-  If the **policy** parameter is not included, the temporary access keys have the same permissions as the IAM user.
-  If the **policy** parameter is included, the temporary access keys' permissions are the intersection of the temporary policy's permissions and the IAM user's permissions.
...
.. figure:: /_static/images/en-us_image_0000002648149962.png
...
.. figure:: /_static/images/en-us_image_0000002678229717.png
...
.. figure:: /_static/images/en-us_image_0000002678389561.jpg
...
#. The customer server has permanent access keys, so it can request IAM to generate different temporary access keys for different apps.
   ...
   IAM users can call the IAM `API to obtain a temporary AK/SK and security token <https://docs.otc.t-systems.com/en-us/api/iam/en-us_topic_0097949518.html>`__. They can also include the **policy** parameter in the request to set a temporary policy. An example is provided as follows:
